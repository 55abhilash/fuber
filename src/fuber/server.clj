...
(defn calcFare [ride_id endLocation startLocation startTime endTime isPink]
  ...
  (def totalFare (+ (* (calcTaxiDistance2 endLocation startLocation) 2) 
...
;  (println (first (jdbc/query dbspec ["select isAssigned from taxi where taxi_id = ?" taxi_id])))

  (if-let [checkTaxi (= (get (first (jdbc/query dbspec ["select isAssigned from taxi where taxi_id = ?" taxi_id])) :isassigned) false)
          ] 
    ...
    ) {:err "This Taxi is no longer available. Run /findTaxi again to find nearest available Taxi."} 
  )
)
