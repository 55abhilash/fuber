...
(defn assignRide [taxi_id start_lat start_long]
  (if-let [checkTaxi (= (get (first (jdbc/query dbspec/dbspec ["select isAssigned from taxi where taxi_id = ?" taxi_id])) :isassigned) false)]
    (try (jdbc/with-db-transaction [t-con dbspec/dbspec]
           ...
           (jdbc/query t-con ["select max(ride_id) as ride_id, taxi_id from ride where taxi_id = ?" taxi_id]))

         (catch Exception e ( println (Throwable->map e)))) {:err "This Taxi is no longer available. Run /findTaxi again to find nearest available Taxi."}))
